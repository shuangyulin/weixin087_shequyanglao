<template>
	<view class="content">
		...
		<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","borderColor":"rgba(242, 241, 241, 1)","margin":"0 0 0px 0","borderStyle":"solid","borderWidth":"0 0 2rpx 0","height":"108rpx"}' v-if="tableName=='fuwurenyuan'" class="cu-form-group">
			<view class="title" :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}'>账号</view>
			<input  style="padding: 0 30upx" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(230, 230, 230, 1)","borderRadius":"0px","color":"rgba(255, 144, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' v-model="ruleForm.zhanghao" placeholder="账号"></input>
		</view>
		<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","borderColor":"rgba(242, 241, 241, 1)","margin":"0 0 0px 0","borderStyle":"solid","borderWidth":"0 0 2rpx 0","height":"108rpx"}' v-if="tableName=='fuwurenyuan'" class="cu-form-group">
		...
		<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","borderColor":"rgba(242, 241, 241, 1)","margin":"0 0 0px 0","borderStyle":"solid","borderWidth":"0 0 2rpx 0","height":"108rpx"}' v-if="tableName=='fuwurenyuan'" class="cu-form-group">
			...
			<view v-if="tableName=='fuwurenyuan'" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"#ccc","margin":"0","borderWidth":"0 0 2rpx 0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
                                <view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">性别</view>
                                <picker @change="fuwurenyuanxingbieChange" :value="fuwurenyuanxingbieIndex" :range="fuwurenyuanxingbieOptions">
                                        <view style="padding: 0 30upx" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(230, 230, 230, 1)","borderRadius":"0px","color":"rgba(255, 144, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input picker-select-input">{{ruleForm.xingbie?ruleForm.xingbie:"请选择性别"}}</view>
                                </picker>
                        </view>
		<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","borderColor":"rgba(242, 241, 241, 1)","margin":"0 0 0px 0","borderStyle":"solid","borderWidth":"0 0 2rpx 0","height":"108rpx"}' v-if="tableName=='fuwurenyuan'" class="cu-form-group">
		...
		<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","borderColor":"rgba(242, 241, 241, 1)","margin":"0 0 0px 0","borderStyle":"solid","borderWidth":"0 0 2rpx 0","height":"108rpx"}' v-if="tableName=='fuwurenyuan'" @tap="fuwurenyuanzhaopianTap" class="cu-form-group" :class="left == 'left'?'':'active'">
			<view class="title" :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}'>照片</view>
			<view style="flex: 1;textAlign:left">
				<image :style='{"width":"88rpx","boxShadow":"0 0 16rpx rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"88rpx"}' v-if="ruleForm.zhaopian" style="margin: 0;" class="avator" :src="ruleForm.zhaopian" mode=""></image>
				<image :style='{"width":"88rpx","boxShadow":"0 0 16rpx rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"88rpx"}' v-else class="avator" style="margin: 0;" src="../../static/center/face.jpeg" mode=""></image>
	...
	export default {
		...
		async onLoad() {
			...
			if(this.tableName=='fuwurenyuan'){
				...
				this.fuwurenyuanxingbieOptions.forEach((item, index) => {
					if(item==this.ruleForm.xingbie) {
						this.fuwurenyuanxingbieIndex = index;
					}
				});
			}
			this.styleChange()
		...
		methods: {
                        ...
                        // 下拉变化
                        fuwurenyuanxingbieChange(e) {
                                this.fuwurenyuanxingbieIndex = e.target.value
                                this.ruleForm.xingbie = this.fuwurenyuanxingbieOptions[this.fuwurenyuanxingbieIndex]
                        },
			styleChange() {
				this.$nextTick(()=>{
			...
			async update() {
				...
				if((!this.ruleForm.zhanghao) && `fuwurenyuan` == this.tableName){
					this.$utils.msg(`账号不能为空`);
					return
				}
				...
				if((!this.ruleForm.xingming) && `fuwurenyuan` == this.tableName){
					this.$utils.msg(`姓名不能为空`);
					return
				}
				if(`fuwurenyuan` == this.tableName && this.ruleForm.shenfenzheng&&(!this.$validate.checkIdCard(this.ruleForm.shenfenzheng))){
					this.$utils.msg(`身份证应输入身份证格式`);
					return
				}
				if(`fuwurenyuan` == this.tableName && this.ruleForm.shouji&&(!this.$validate.isMobile(this.ruleForm.shouji))){
					this.$utils.msg(`手机应输入手机格式`);
					return
				}
				if(`fuwurenyuan` == this.tableName && this.ruleForm.youxiang&&(!this.$validate.isEmail(this.ruleForm.youxiang))){
					this.$utils.msg(`邮箱应输入邮件格式`);
					return
				}
				let table = uni.getStorageSync("nowTable");
