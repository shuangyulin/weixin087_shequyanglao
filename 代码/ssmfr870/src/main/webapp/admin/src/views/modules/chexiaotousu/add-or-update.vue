<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="服务员" prop="fuwuyuan">
          <el-input v-model="ruleForm.fuwuyuan" 
              placeholder="服务员" clearable  :readonly="ro.fuwuyuan"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="服务员" prop="fuwuyuan">
              <el-input v-model="ruleForm.fuwuyuan" 
                placeholder="服务员" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fuwumingcheng = true;
          ...
          if(o=='fuwuyuan'){
            this.ruleForm.fuwuyuan = obj[o];
	    this.ro.fuwuyuan = true;
            continue;
          }
          if(o=='fuwushijian'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `chexiaotousu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.chexiaotousuCrossAddOrUpdateFlag = false;
                  this.parent.search();
